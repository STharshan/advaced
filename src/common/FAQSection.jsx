...
const FaqItem = ({ question, answer }) => {
  ...
  return (
    <div
      ...
      className={`group flex flex-col gap-2 p-5 rounded-[14px] bg-[#08060F] border transition-all duration-300 cursor-pointer ${isOpen ? "border-[#7C2FC0]" : "border-[rgba(184,192,204,0.25)] hover:border-[#7C2FC0]"
        }`}
    >
      <div className="flex items-center justify-between gap-4">
        <span className="text-white text-[15px] font-medium leading-6">{question}</span>
        <div className={`w-8 h-8 min-w-8 rounded-full border flex items-center justify-center transition-colors duration-200 ${isOpen ? "bg-[#7C2FC0] border-[#7C2FC0] text-white" : "border-[rgba(184,192,204,0.35)] text-[#B8C0CC]"
          }`}>
      ...
      <div className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? "max-h-50 opacity-100 mt-2" : "max-h-0 opacity-0"
        }`}>
